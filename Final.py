...
df = pd.read_csv(DATA_PATH)   # pandas auto-detects .gz
...
def ensure_canonical_columns(df):
    ...
    df = df.copy()
    alias_map = {
        'air_quality_pm2.5': ['pm2.5', 'pm25', 'pm2_5', 'pm2_5_air_quality', 'pm25_value', 'air_quality'],
        'precip_mm': ['precipitation_mm', 'precipitation', 'precip', 'rain', 'rainfall'],
        'wind_kph': ['wind_speed_kph', 'wind_speed', 'wind', 'windspeed'],
        'temperature_celsius': ['temp_c', 'temp', 'temperature', 'air_temp'],
    ...
    # As final fallback, if still missing, add with reasonable default (so .mean() won't crash)
    defaults = {
        'air_quality_pm2.5': 50.0,
        'precip_mm': 0.0,
        'wind_kph': 10.0,
...
def add_derived_metrics(df):
    # --- ensure datetime & month/year exist (defensive, central place) ---
    if 'last_updated_dt' not in df.columns or df['last_updated_dt'].isnull().all():
        df['last_updated_dt'] = pd.to_datetime(df.get('last_updated', df.get('last_updated_dt')), errors='coerce')

    # create year/month/year_month columns (safe)
    df['year'] = df['last_updated_dt'].dt.year
    df['month'] = df['last_updated_dt'].dt.month
...
def create_extreme_events_timeline(df, threshold_dict, title="Extreme Events Over Time", height=400):
    """Create line chart of extreme event counts over months — safe when columns missing"""
    df = df.copy()
    df['year_month'] = df['last_updated_dt'].dt.to_period('M').astype(str)

    # Prepare ordered months (even if some months have zero events)
    months = sorted(df['year_month'].unique())

    monthly_extremes = {}
    for event_name, (col, threshold, op) in threshold_dict.items():
        # If column missing, create zero series so code won't crash
        if col not in df.columns:
            counts = pd.Series(0, index=months)
        else:
            if op == '>':
                counts = df[df[col] > threshold].groupby('year_month').size()
            else:
                counts = df[df[col] < threshold].groupby('year_month').size()

            # Reindex to include all months with zeros where missing
            counts = counts.reindex(months, fill_value=0)

        monthly_extremes[event_name] = counts
...
def show_global_analysis(df, filters):
    ...
    with col1:
        ...
        if filters['show_rangeslider'] and x_col == 'date':
            layout_updates['xaxis']['rangeslider'] = dict(visible=True, thickness=0.1)
        fig.update_layout(height=400, xaxis_title=x_col.title(), yaxis_title=f'{metric_label} ({metric_unit})',
                          **layout_updates)
    ...
    with col1:
        ...
        if not filters['normalize']:
            st.info(
                f"Most observations are between {metric_data.quantile(0.25):.2f} and {metric_data.quantile(0.75):.2f} {metric_unit}")
    ...
    with col2:
        ...
        if 'country' in df.columns and df['country'].nunique() <= 20:
            ...
            fig = create_box_plot(df_sample, 'country', display_col, f'Box Plot - {metric_label} (Top 10 Countries)',
                                  height=400)
        render_plotly_chart(fig)
    ...
    with col1:
        ...
        if 'country' in df.columns and df['country'].nunique() <= 15:
            ...
            top_countries = df.groupby('country').size().nlargest(10).index
            df_sample = df[df['country'].isin(top_countries)]
            fig = create_violin_plot(df_sample, 'country', display_col,
                                     f'Violin Plot - {metric_label} (Top 10 Countries)', height=400)
        render_plotly_chart(fig)
    ...
    if df['country'].nunique() >= 3:
        ...
        fig = create_ridgeline_plot(df_ridge, display_col, 'country', f'Ridgeline Plot - {metric_label} by Country',
                                    height=600)
        render_plotly_chart(fig)
    ...
    if extremes:
        ...
        if extremes:
            ...
            st.warning(
                f"Most Common: {max_event} with {extremes[max_event]:,} events ({extremes[max_event] / len(df) * 100:.1f}% of data)")
...
def show_country_analysis(df, filters):
    ...
    st.markdown(
        f"Analyzing **{len(country_df):,}** observations from **{country_df['location_name'].nunique()}** locations")
    ...
    with col1:
        ...
        if not filters['normalize']:
            ...
            if len(metric_data.mode()) > 0:
                st.caption(
                    f"Total observations: {len(metric_data)} | Most common: {metric_data.mode().values[0]:.2f} {metric_unit}")
    ...
    if 'wind' in metric_col:
        ...
        extreme_low = len(country_df[
                              country_df[display_col] < -threshold if 'temperature' in metric_col else country_df[
                                                                                                           display_col] < threshold])
...
def show_executive_dashboard(df, filters):
    ...
    with insights_col1:
        ...
        st.info(
            f"**Hottest Region:** {hottest_country} ({df[df['country'] == hottest_country]['temperature_celsius'].mean():.1f}°C avg)")
        st.info(
            f"**Coldest Region:** {coldest_country} ({df[df['country'] == coldest_country]['temperature_celsius'].mean():.1f}°C avg)")

        st.markdown("#### 🌧️ Precipitation Insights")
        rainiest_country = df.groupby('country')['precip_mm'].mean().idxmax()
        st.info(
            f"**Rainiest Region:** {rainiest_country} ({df[df['country'] == rainiest_country]['precip_mm'].mean():.2f} mm avg)")

    with insights_col2:
        st.markdown("#### 💨 Wind Insights")
        windiest_country = df.groupby('country')['wind_kph'].mean().idxmax()
        st.info(
            f"**Windiest Region:** {windiest_country} ({df[df['country'] == windiest_country]['wind_kph'].mean():.1f} km/h avg)")
        ...
        if 'air_quality_pm2.5' in df.columns:
            ...
            st.warning(f"**Worst Air Quality:** {worst_aq_country}")
            st.success(f"**Best Air Quality:** {best_aq_country}")
        else:
            st.info("Air quality data not available in this dataset")
...
def show_statistical_analysis(df, filters):
    ...
    stats_data = {
        ...
        'Metric': [metric_label, 'Temperature (°C)', 'Humidity (%)', 'Precipitation (mm)', 'Wind Speed (km/h)',
                   'PM2.5'],
        'Mean': [
            df[metric_col].mean(),
    ...
    with col1:
        ...
        country_temp = df[df['country'].isin(top_countries)].groupby('country')[
            'temperature_celsius'].mean().sort_values(ascending=False)
        fig = px.bar(
    ...
    with col2:
        # Humidity comparison
        country_humidity = df[df['country'].isin(top_countries)].groupby('country')['humidity'].mean().sort_values(
            ascending=False)
        fig = px.bar(
    ...
    with col1:
        ...
        country_wind = df[df['country'].isin(top_countries)].groupby('country')['wind_kph'].mean().sort_values(
            ascending=False)
        fig = px.bar(
    ...
    with col2:
        # PM2.5 comparison
        country_pm25 = df[df['country'].isin(top_countries)].groupby('country')['air_quality_pm2.5'].mean().sort_values(
            ascending=False)
        fig = px.bar(
...
def show_climate_trends(df, filters):
    ...
    # defensive: ensure month exists for filtered df
    if 'last_updated_dt' not in df.columns or df['last_updated_dt'].isnull().all():
        df['last_updated_dt'] = pd.to_datetime(df.get('last_updated', df.get('last_updated_dt')), errors='coerce')

    if 'month' not in df.columns:
        df['month'] = df['last_updated_dt'].dt.month.fillna(0).astype(int)
    if 'year' not in df.columns:
        df['year'] = df['last_updated_dt'].dt.year.fillna(0).astype(int)
...
def create_extreme_events_tables(df):
    """Create top-5 extreme events tables"""
    df = df.copy()
    ...
    # 🔥 Hottest days
    tables['hottest'] = (
        df.sort_values('temperature_celsius', ascending=False)
        [['last_updated_dt', 'country', 'location_name', 'temperature_celsius']]
        .head(5)
        .rename(columns={'temperature_celsius': 'Temperature (°C)'})
    )
    ...
    # 💨 Windiest days
    tables['windiest'] = (
        df.sort_values('wind_kph', ascending=False)
        [['last_updated_dt', 'country', 'location_name', 'wind_kph']]
        .head(5)
        .rename(columns={'wind_kph': 'Wind Speed (km/h)'})
    )

    # 🌧️ Rainiest days
    tables['rainiest'] = (
        df.sort_values('precip_mm', ascending=False)
        [['last_updated_dt', 'country', 'location_name', 'precip_mm']]
        .head(5)
        .rename(columns={'precip_mm': 'Precipitation (mm)'})
    )
...
def show_help_page():
    ...
    st.markdown("---")
    st.info(
        "💡 **Tip**: Use the sidebar filters to customize your analysis. All charts update automatically based on your selections!")
...
def show_data_processing_page(df):
    ...
    steps = [
        ...
        ("✅", "**Variable Renaming**",
         "Added descriptive aliases (precipitation_mm, wind_speed_kph, pm2_5_air_quality)"),
    ...
    st.markdown("---")
    st.info(
        "💡 **Tip**: Use the download buttons to export aggregated datasets for further analysis in Excel or other tools!")
...
def main():
    ...
    page = st.sidebar.radio(
        "Navigate to:",
        ["Executive Dashboard", "Statistical Analysis", "Climate Trends", "Extreme Events", "Agriculture Analysis",
